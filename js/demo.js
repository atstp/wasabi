...
var transitionOpacity = (function(){
  ...
  var output = document.getElementById('output-area');
  var distanceToCovering = banners.offsetTop;
  var distanceToPeeking = distanceToCovering - window.innerHeight;
  var setOpacity = function(){
    if(window.scrollY < (distanceToPeeking + 30)) { // + buffer
      output.style.opacity = 1;
    } else if(window.scrollY < distanceToCovering) {
      output.style.opacity = (distanceToCovering-window.scrollY)/window.innerHeight;
    } else {
...
setInterval(transitionOpacity, 500);
